/*
    Query Store prerequisites. Read-only, changes nothing.

    Answers one question for every user database on the instance. Can I
    turn Query Store on here, and if not, what is stopping me.

    Result 1
        Instance overview. Facts that apply to every database.

    Result 2
        Database overview. One row per database.

    Result 3
        Recommendations. Only problems show up. A database with nothing
        wrong gets one Ready row. Scope is the database the finding is
        about, or Instance Check when it applies to the whole instance.

    Severity
        Blocker   Fix this or 02_Turn_On.sql will fail or do nothing useful
        Warning   Query Store will run, but it will not stay healthy
        Note      Worth knowing, nothing to fix
        Ready     Nothing blocking Query Store on this database

    System databases are skipped. Run 02_Turn_On.sql next.
*/
...
-- a WHILE loop over the key, not a cursor.
