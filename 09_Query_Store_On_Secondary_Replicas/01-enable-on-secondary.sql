/*
    01 - Enable Query Store for readable secondary replicas (SQL Server 2025+)
    ...
    readable secondary, not just the replica you're connected to, and
    Microsoft documents this as limited preview on 2022 -- NOT supported
    in production. SQL Server 2025+ doesn't need this step.

    Run this entire script while connected to the PRIMARY replica.
*/

USE master;
GO

-- 0. SQL SERVER 2022 ONLY. Run this on the primary AND on every readable
--    secondary (connect to each one separately -- DBCC TRACEON doesn't
--    propagate across replicas). -1 enables it instance-wide; it resets
--    on service restart, so re-run after any restart, or set -T12606 as
--    a startup parameter in SQL Server Configuration Manager for it to
--    survive restarts (that requires restarting the service once).
--    Skip this step entirely on SQL Server 2025+.
-- DBCC TRACEON (12606, -1);
-- GO

-- 1. Query Store must be on and READ_WRITE on the primary first.
--    No-op if it's already enabled (see 01_Setup for the full setup).
ALTER DATABASE [AdventureWorks2022]
    SET QUERY_STORE = ON (OPERATION_MODE = READ_WRITE);
GO

-- 2. Enable Query Store capture on every readable secondary replica.
--    This is issued from the primary and applies across the replica set.
ALTER DATABASE [AdventureWorks2022]
    FOR SECONDARY
    SET QUERY_STORE = ON
...
-- 3. Optional: let automatic tuning force the last-known-good plan on
