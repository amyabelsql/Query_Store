...
Everything Query Store collects is queryable through catalog views in the user database, no external tooling required. Run [01_Explore_Catalog_Views.sql](01_Explore_Catalog_Views.sql) after generating the workload.
...
| `sys.query_store_query` | One row per captured query, including parameterization type, object it belongs to, compile stats |
...
## Baseline query for everything about a query
...
Key columns in `sys.query_store_runtime_stats` you'll use constantly are `avg_duration`, `avg_cpu_time`, `avg_logical_io_reads`, `avg_query_max_used_memory`, `count_executions`, `execution_type_desc`, and `last_execution_time`. All time values are in **microseconds**. I/O values are in **8-KB pages**.
...
- SQL Server 2016 to 2019 requires `VIEW DATABASE STATE`.
- SQL Server 2022+ requires `VIEW DATABASE PERFORMANCE STATE` (or a broader permission like `VIEW DATABASE STATE`).

Continue with [04_Find_And_Fix_Regressions](../04_Find_And_Fix_Regressions/).
