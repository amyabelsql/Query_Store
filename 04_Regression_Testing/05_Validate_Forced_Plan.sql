/*
	...
	Output is compact: meaningful rows, no wide scrolling.
*/

USE [AdventureWorks2022];
GO

DECLARE @QueryId BIGINT = 44; -- Query Id from step 3
DECLARE @ForcedPlanId BIGINT = 44; -- Plan Id forced in step 4
...
-- Sink table suppresses repeated stored procedure rowsets in output.
...
-- Baseline is the forced plan's historical averages.
...
-- Regressed is the worst non-forced alternative plan (distinct from baseline).
...
SELECT
	N'Comparison' AS [Section],
	N'Duration Improvement Vs Regressed (%)' AS [Metric],
...
SELECT
	N'Comparison' AS [Section],
	N'Reads Improvement Vs Regressed (%)' AS [Metric],
