...
-- Restore supporting indexes so good plan is possible again
IF NOT EXISTS
(
	SELECT 1
	FROM sys.indexes
...
-- Find candidate plans. Aggregates runtime stats so each plan appears once.
-- Default behavior picks the most recently executed matching query_id.
DECLARE @QueryTextPattern VARCHAR(200) = '%WHERE d.ProductID = @ProductID%'; -- text pattern to find demo query
DECLARE @TargetQueryId BIGINT = NULL; -- Optional: set to a specific query_id to evaluate
